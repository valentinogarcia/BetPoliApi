...
app.post('/users/validate/dni', async (_req:any,_res:any)=>{let COLLECTION_NAME:string; if(_req.body.type == "user"){ COLLECTION_NAME = COLLECTION_USER }else{COLLECTION_NAME=COLLECTION_REPORTER} const dni = _req.body.dni ; const x = getUserByDNI(await db.collection(COLLECTION_NAME).find().toArray(),dni); x!.accept();_res.send(await db.collection(COLLECTION_NAME).findOneAndReplace( {"dni":x!.getDNI() },x! )); } )
app.post('/users/invalidate/dni',async (_req:any,_res:any) => {let COLLECTION_NAME:string; if(_req.body.type == "user"){ COLLECTION_NAME = COLLECTION_USER }else{COLLECTION_NAME=COLLECTION_REPORTER} const dni = _req.body.dni ; const x = getUserByDNI(await db.collection(COLLECTION_NAME).find().toArray(),dni);x!.deny(); _res.send(await db.collection(COLLECTION_NAME).findOneAndReplace( {"dni":x!.getDNI() },x! ));  })
app.post('/users/pend/dni',async (_req:any,_res:any) => {let COLLECTION_NAME:string; if(_req.body.type == "user"){ COLLECTION_NAME = COLLECTION_USER }else{COLLECTION_NAME=COLLECTION_REPORTER} const dni = _req.body.dni ; const x = getUserByDNI(await db.collection(COLLECTION_NAME).find().toArray(),dni);x!.pend(); _res.send(await db.collection(COLLECTION_NAME).findOneAndReplace( {"dni":x!.getDNI() },x! ));  })
